    ...
    <div className="min-h-screen bg-slate-950 text-white font-sans antialiased selection:bg-[#B2EA4D]/30 selection:text-slate-200">
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-6 py-20 text-center space-y-16">
        <div className="space-y-4 max-w-2xl mx-auto">
          <span className="text-xs font-bold text-[#B2EA4D] uppercase tracking-widest bg-[#B2EA4D]/10 px-3 py-1 rounded-full border border-[#B2EA4D]/20">Pricing Plans</span>
          ...
          {[
            ...
            <div 
              ...
              className={`relative border rounded-2xl p-8 flex flex-col justify-between transition-all duration-300 ${
                plan.popular 
                  ? "border-[#B2EA4D]/50 bg-slate-900/40 shadow-xl shadow-teal-500/5 scale-[1.03]" 
                ...
                <ul className="space-y-3.5 text-xs text-slate-300 font-medium">
                  {plan.features.map((f, i) => (
                    <li key={i} className="flex items-center gap-2.5">
                      <Check className="w-4 h-4 text-[#B2EA4D] shrink-0" />
                      <span>{f}</span>
                    </li>
                  ))}
                </ul>
              ...
              <a href={plan.price === "Custom" ? "/contact" : "/register"} className="block mt-8 w-full">
                <Button 
                  className={`w-full h-11 font-bold rounded-xl flex items-center justify-center gap-1.5 transition-all ${
                    plan.popular 
                      ? "bg-[#B2EA4D] hover:bg-teal-400 text-slate-950" 
