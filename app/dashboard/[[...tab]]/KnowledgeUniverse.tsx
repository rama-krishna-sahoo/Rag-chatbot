...
// Curated Sleek Palette for Semantic Communities
...
export function getCategoryColor(cat: string): string {
  if (!cat || cat.toLowerCase() === "query") return "#ffffff";
  if (cat.toLowerCase() === "general") return "#667a61";
  ...
  const index = Math.abs(hash) % (VibrantPalette.length - 1);
  return VibrantPalette[index];
}

// Dynamic color lookup — no hardcoded categories, everything is derived from actual data
export const ClusterColors: Record<string, string> = new Proxy({} as Record<string, string>, {
  get(_target, prop: string) {
    return typeof prop === "string" ? getCategoryColor(prop) : "#B2EA4D";
  }
});
...
export function KnowledgeUniverse() {
  ...
  const activeClusterCategories = useMemo(() => {
    const cats = new Set<string>();
    nodes.forEach(n => {
      if (n.category && n.category !== "query" && n.nodeType !== "keyword") {
        cats.add(n.category);
      }
    });
    // Only show categories that actually exist in the data — no hardcoded fallback
    ...
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 lg:h-[calc(100vh-10rem)] min-h-[700px] lg:min-h-0 animate-mac-page">
      ...
      <Card className="lg:col-span-3 bg-[#16250e]/60 backdrop-blur border-[#B2EA4D]/15 rounded-xl overflow-hidden relative flex flex-col h-full border shadow-2xl">
        ...
        <div className="absolute bottom-4 left-4 right-4 z-10 flex flex-col sm:flex-row gap-3 justify-between pointer-events-none">
          {/* Legend Panel - only shown when categories actually exist in loaded data */}
          {activeClusterCategories.length > 0 && (
            <div className="bg-[#0c1407]/85 backdrop-blur border border-[#B2EA4D]/15 p-3 rounded-lg flex flex-col gap-2 max-w-xs select-none pointer-events-auto shadow-xl">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-[9px] font-semibold text-slate-300">
                {activeClusterCategories.map((cat) => (
                  <div key={cat} className="flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: ClusterColors[cat] || getCategoryColor(cat) }} />
