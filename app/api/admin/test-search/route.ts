...
export async function POST(req: Request) {
  try {
    ...
    const targetWs = (workspaceId && workspaceId !== "ffffffff-ffff-ffff-ffff-ffffffffffff") ? workspaceId : null;

    let { data: matches, error: rpcError } = await supabase.rpc(
      "match_knowledge",
      {
        query_embedding: queryEmbedding,
        filter_workspace_id: targetWs,
        match_count: matchCount || 5,
        filter_category: category || null,
        filter_status: status || null // Supports draft, published, or all
      }
    );

    if ((!matches || matches.length === 0) && targetWs) {
      const { data: globalMatches } = await supabase.rpc(
        "match_knowledge",
        {
          query_embedding: queryEmbedding,
