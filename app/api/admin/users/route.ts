  ...
  try {
    ...
    if (createError) {
      ...
      if (createError.message.includes("already registered") || createError.message.includes("already exists")) {
        ...
        if (existingUser) {
          // Security: Prevent modifying a Super Admin user or poaching users from other workspaces
          ...
          if (targetUserRole) {
            ...
            if (targetUserRole.workspace_id && targetUserRole.workspace_id !== workspaceId && callerRole !== 'Super Admin') {
              return NextResponse.json({ 
                error: "This user already belongs to another isolated workspace and cannot be added." 
              }, { status: 400 });
            }
          }
