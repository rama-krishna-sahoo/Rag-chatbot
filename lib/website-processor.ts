...
export function inferCategoryFromUrlOrTitle(url: string, title?: string): string | null {
  const text = `${url} ${title || ''}`.toLowerCase();
  
  if (text.includes("shipping") || text.includes("return") || text.includes("exchange") || text.includes("delivery") || text.includes("tracking")) {
    return "Shipping & Returns";
  }
  if (text.includes("privacy") || text.includes("terms") || text.includes("policy") || text.includes("condition") || text.includes("legal")) {
    return "Policies & Legal";
  }
  if (text.includes("faq") || text.includes("help") || text.includes("support") || text.includes("contact")) {
    return "Help & Support";
  }
  if (text.includes("about") || text.includes("story") || text.includes("mission") || text.includes("team")) {
    return "About Us";
  }
  if (text.includes("pricing") || text.includes("plans") || text.includes("billing") || text.includes("subscription")) {
    return "Pricing & Plans";
  }

  // Check URL pathname for collection/category patterns
  try {
    const parsed = new URL(url);
    const pathParts = parsed.pathname.split("/").filter(Boolean);
    const catIndex = pathParts.findIndex(p => ["collections", "collection", "category", "categories", "c", "shop", "products"].includes(p.toLowerCase()));
    if (catIndex !== -1 && pathParts[catIndex + 1]) {
      const rawCategory = pathParts[catIndex + 1].replace(/[-_]+/g, " ");
      return rawCategory.split(" ").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(" ");
    }
  } catch {}
...
export async function processUrlForWorkspace(
  ...
  try {
    ...
    const inferredCategory = inferCategoryFromUrlOrTitle(url, features.title);
    const finalCategory = (features.category && features.category.toLowerCase() !== "general")
      ? features.category
